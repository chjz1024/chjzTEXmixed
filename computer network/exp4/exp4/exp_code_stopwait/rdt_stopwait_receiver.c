...
int receive_file( char *save_file_name, int sock_fd )
{
	...
	{			
		...
		*/
		pkt_len = recvfrom(sock_fd, rdt_pkt, RDT_PKT_LEN, MSG_WAITALL, (struct sockaddr *)&client_addr, (socklen_t *)&sin_len);
        ...
        if (flag == RDT_CTRL_END) {
            ...
            udt_sendto(sock_fd,reply_pkt_buf,reply_pkt_len,MSG_CONFIRM,(struct sockaddr *)&client_addr, sin_len);
            break;
            ...
            reply_pkt_len = pack_rdt_pkt(NULL, reply_pkt_buf, 0, exp_seq_num, RDT_CTRL_NACK);
        }
        udt_sendto(sock_fd,reply_pkt_buf,reply_pkt_len,MSG_CONFIRM,(struct sockaddr *)&client_addr, sin_len);
	}
